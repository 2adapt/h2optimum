...
The file \`cookies.txt\` (the "cookie jar") should contain the session data in a cookie named \`sid\`.
	...
	server.route({
		...
		handler: async function (request, h) {
			...
			let result = await internals.fetchMeasurements(request, h);	
			return result;
		}
	});


	description = 'Export a list of measurements as CSV';
	notes = `
...
curl ${process.env.API_ORIGIN}/api/v2/measurement-export?device_mac=00-00-00-00-00-00&installation_id=123&from_date=2023-01-01&&to_date=2023-01-05 \
--request GET \
--cookie cookies.txt
...
curl ${process.env.API_ORIGIN}/api/v2/measurement-export?device_mac=00-00-00-00-00-00&installation_id=123&from_date=2023-01-01&&to_date=2023-01-05&limit=5000&potential_threshold=600 \
--request GET \
--cookie cookies.txt
...
curl ${process.env.API_ORIGIN}/api/v2/measurement-export?device_mac=00-00-00-00-00-00&installation_id=123&hours=6 \
--request GET \
--cookie cookies.txt
...
The file \`cookies.txt\` (the "cookie jar") should contain the session data in a cookie named \`sid\`.
	...
	server.route({
		method: 'GET',
		path: '/api/v2/measurement-export',
		options: {
			auth: {
			    mode: 'try',
			    strategy: 'session',
		...
		handler: async function (request, h) {
			...
			// explicitly give the the order of the columns for the csv
			...
			let response = h.response(csv);

			// prepare filename

			let installationName = await sql`
				select slugify(name) as name
				from t_installations
				where id = ${request.query.installation_id}
			`;

			let fromDate = request.query.from_date.toISOString().replace(/-/g, '').split('T')[0];
			let toDate = request.query.to_date.toISOString().replace(/-/g, '').split('T')[0];
			let deviceMac = request.query.device_mac.replace(/-|:/g, '');
			let basename = installationName[0]['name'] + '__' + fromDate + '__' + toDate + '__' + deviceMac;
			if (request.query.time_bucket != null) {
				basename = basename + '__' + request.query.time_bucket + 'h';
			}
			
			// console.log({ basename })
			response.header('Content-Type', 'text/csv');
			response.header('Content-Disposition', `attachment; filename=${basename}.csv`);
...
internals.fetchMeasurements = async function fetchMeasurements (request, h) {
