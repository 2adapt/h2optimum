import { c as create_ssr_component, s as subscribe, a as each, e as escape, b as add_attribute, v as validate_component } from "../../../../chunks/index.js";
/* empty css                          */import { p as page } from "../../../../chunks/stores.js";
import "devalue";
import { h as hasErrors } from "../../../../chunks/utils.js";
const CardsGrid = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  $$unsubscribe_page();
  return `<ul class="grid grid-cols-1 gap-6 py-4 sm:grid-cols-2 lg:grid-cols-3">
	${each(cards, (card) => {
    return `<li class="col-span-1 flex flex-col divide-y divide-gray-200 rounded-lg bg-white shadow">
				<div class="min-h-[90px] border-b border-gray-200 bg-white px-4 py-5 sm:px-6">
					<div class="-ml-4 -mt-4 flex flex-nowrap items-center justify-between">
						<a href="${"/backoffice/installations/" + escape(card.slug, true) + "-" + escape(card.id, true)}" class="group">
							<div class="ml-4 mt-4">
								<h3 class="text-base font-semibold leading-6 text-gray-900 group-hover:text-sky-500">${escape(card.name)}</h3>

								<p class="mt-1 line-clamp-1 text-sm text-gray-500">${card.description ? `${escape(card.description)}` : ``}</p>
							</div>
						</a>
						<div class="relative inline-block text-left">
							...
							<div data-transition-enter="transition ease-out duration-100" data-transition-enter-start="transform opacity-0 scale-50" data-transition-enter-end="transform opacity-100 scale-100" data-transition-leave="transition ease-in duration-75" data-transition-leave-start="transform opacity-100 scale-100" data-transition-leave-end="transform opacity-0 scale-95" class="absolute right-0 z-10 mt-2 w-56 origin-top-right rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none" role="menu" aria-orientation="vertical" aria-labelledby="menu-button" tabindex="-1">
								...
								<form class="group flex items-center px-4 py-2 text-sm text-gray-700" method="POST" action="${escape(currentPath, true) + "?/deleteInstallation"}">
								...
								<button type="submit" class="group flex items-center px-3 py-2 text-sm text-gray-700">Apagar
								</button>
								<input type="hidden" id="ID" name="ID"${add_attribute("value", card.id, 0)}> 
								</form>
			...
			<div class="z-0 h-56 w-full flex-shrink-0"${add_attribute("this", card.mapBlock, 0)}></div>
			<div>
				<div class="-mt-px flex divide-x divide-gray-200">
					<div class="flex w-0 flex-1">
						<div class="relative -mr-px inline-flex w-0 flex-1 items-center justify-center gap-x-3 rounded-bl-lg border border-transparent py-4 text-sm font-semibold text-gray-900">
							${card && card.status == 0 ? `<svg class="h-5 w-5 text-red-400" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor"><path d="M11.645 20.91l-.007-.003-.022-.012a15.247 15.247 0 01-.383-.218 25.18 25.18 0 01-4.244-3.17C4.688 15.36 2.25 12.174 2.25 8.25 2.25 5.322 4.714 3 7.688 3A5.5 5.5 0 0112 5.052 5.5 5.5 0 0116.313 3c2.973 0 5.437 2.322 5.437 5.25 0 3.925-2.438 7.111-4.739 9.256a25.175 25.175 0 01-4.244 3.17 15.247 15.247 0 01-.383.219l-.022.012-.007.004-.003.001a.752.752 0 01-.704 0l-.003-.001z"></path></svg>` : `<svg class="h-5 w-5 text-green-400" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor"><path d="M11.645 20.91l-.007-.003-.022-.012a15.247 15.247 0 01-.383-.218 25.18 25.18 0 01-4.244-3.17C4.688 15.36 2.25 12.174 2.25 8.25 2.25 5.322 4.714 3 7.688 3A5.5 5.5 0 0112 5.052 5.5 5.5 0 0116.313 3c2.973 0 5.437 2.322 5.437 5.25 0 3.925-2.438 7.111-4.739 9.256a25.175 25.175 0 01-4.244 3.17 15.247 15.247 0 01-.383.219l-.022.012-.007.004-.003.001a.752.752 0 01-.704 0l-.003-.001z"></path></svg>`}
						</div>
					</div>
...
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  if ($$props.data === void 0 && $$bindings.data && data !== void 0)
    $$bindings.data(data);
  return `<main class="flex-1 overflow-y-auto Xborder-[5px] Xborder-green-400 scrollbar-custom overscroll-auto">
	<div class="max-w-full 2xl:max-w-[96rem] mx-auto">
		<div class="px-4 sm:px-6 lg:px-8 Xborder Xborder-orange-400">
			...
			${validate_component(CardsGrid, "CardsGrid").$$render($$result, { cards: data.Cards }, {}, {})}
		</div>
	</div>
</main>`;
});
